module Graphics {
    ...
    export class Arrow extends Line {
        ...
        public draw(contex: CanvasRenderingContext2D) {
            ...
            contex.beginPath();
            contex.moveTo(this.x, this.y);
            contex.lineTo(this.x, this.y1);
            contex.moveTo(this.x, this.y);
            contex.lineTo(this.x - 5, this.y + 5);
            contex.moveTo(this.x, this.y);
            contex.lineTo(this.x + 5, this.y + 5);
            contex.closePath();
            contex.stroke();
    ...
    export class SplashScreen {
        ...
        constructor(public width: number, public height: number, public canvas: HTMLCanvasElement) {
            ...
            this.circle = new Circle(this.centerx - 20, this.centery, 40, "black");
        ...
        private drawTriangle(contex: CanvasRenderingContext2D, color: string) {
            ...
            contex.lineWidth = 2.0;
            contex.beginPath();
            contex.moveTo(this.centerx - 30, this.centery - 17);
            contex.lineTo(this.centerx, this.centery);
            contex.lineTo(this.centerx - 30, this.centery + 17);
            contex.lineTo(this.centerx - 30, this.centery - 17);
            contex.closePath();
            contex.stroke();
        }
        public writeTitle() {
            ...
            this.ctx.fillText("点击开始", this.centerx - 80, this.centery + 80);
        }
        public Show() {
    ...
    export class State {
        ...
        constructor(public width: number, public height: number, public canvas: HTMLCanvasElement) {
            ...
            this.canvas.width = this.width;
            this.canvas.height = this.height;
            this.arrow = new Graphics.Arrow(30, 260, 30, 280, "red");
            this.arrowSprite = new Graphics.Sprite(this.ctx, this.arrow, this.arrow.x, this.arrow.y, this.arrow.x, this.arrow.y, 2, 5, new Date(), 0, true);
        ...
        public loop(): boolean {
            ...
            this.drawArrow();
            this.drawMinText();
            return this.isNodesArrived() && this.isArrowArrived();
        }

        private drawArrow() {
            this.arrowSprite.redraw();
        }

        private drawMinText() {
            this.ctx.fillText("MIN", 15 + 30 * this.minindex, 280);
        }

        private isArrowArrived(): boolean {
            if (this.arrowSprite.changed) {
                return this.arrowSprite.arrived;
            } else {
                return true;
            }
        }
        ...
        public async Selection() {
            ...
            for (let i = 0; i < this.MAX_NODE_LEN; i++) {
                ...
                for (let j = i; j < this.MAX_NODE_LEN; j++) {
                    await this.moveArrow(j);
                    if (this.less(j, min)) {
                        await this.unselected(min);
                        min = j;
                        await this.selected(min);
                    }
                }
        ...
        private setColor(i: number, color: string) {
            const ia = this.insertArray[i];
            const na = this.nodes[ia];
            na.setColor(color);
        }

        private async moveArrow(i: number) {
            this.arrowSprite.endx = 24 + i * 30;
            this.arrowSprite.setChanged();
            this.arrowSprite.xspeed = 2;
            await this.wapper.start();
        }

        private async selected(i: number) {
            this.minindex = i;
            this.setColor(i, "lightgreen");
        }

        private async unselected(i: number) {
        ...
        public async swap(a: number, b: number) {
            ...
            na.setColor("red");
            nb.setColor("red");
            na.movetopoint(nb_col, na.row);
            nb.movetopoint(na_col, na.row);
            await this.wapper.start();
            na.setColor("blue");
            nb.setColor("black");
            this.drawbackground();
